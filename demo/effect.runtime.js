...
export default function Runtime(action = null) {
...
Runtime.prototype.run = function() {
  if (this._action) {
    ...
    this._action.queue = this._action.queue.catch(
      this._action.onProcessError.bind(this._action)
    );

    // Concat the built effect after the accumulating.
    this._action.queue = this._action.queue.then(() => {
      // Get the result from the ended action.
      var data = this._action.result;
